...
export class FilesListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  ngOnInit() {
    ...
    this.filterSettings$ = this.store
      .select(selectFilterSettings)
      .pipe(takeUntil(this.onDestroy$));

    // data source setup
    this.getSlicesFiles().subscribe((files) =>
      this.ngZone.run(() => (this.dataSource.data = files)),
    );
  ...
  getSlicesFiles() {
    const hasScrolledToBottom$: Observable<boolean> = this.scroll
      .scrolled()
      .pipe(
        map(
        ...
        map(
          (scrollBottom) =>
            document.documentElement.scrollHeight - 50 <= scrollBottom,
        ),
    ...
    this.shouldLoadAllData$ = combineLatest(
      hasScrolledToBottom$,
      hasAFilter$,
    ).pipe(
    ...
    const slicedFiles$ = combineLatest(
      this.files$,
      this.shouldLoadAllData$,
      this.sortSettings$,
    ).pipe(
  ...
  sortChange(sort: Sort) {
    ...
    this.store.dispatch(
      new ChangeSortSettings({ sortAttribute: sort.active, sortOrder }),
    );
  }
